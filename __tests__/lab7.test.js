describe('Basic user flow for Website', () => {
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
     ...
     */
      ...
      // Evaluate localStorage in the page context
      const cart = await page.evaluate(() => {
        return localStorage.getItem('cart');
      });

      // The expected cart value after adding all 20 items
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    while (i < prodItems.length) {
      const shadowRoot = await prodItems[i].getProperty('shadowRoot');
      const shadow = await shadowRoot.asElement();

      const button = await shadow.$('button'); // Corrected from shadowRoot to shadow
...
expect(cartCount).toBe('0');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartCountProperty = await cartCountElem.getProperty('innerText');
    const cartCount = await cartCountProperty.jsonValue();
